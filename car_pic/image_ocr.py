...
#GPU--动态申请显存
os.environ['CUDA_VISIBLE_DEVICES'] = '0' #选定特定GPU训练
        ...
        with codecs.open(self.monogram_file, mode='r', encoding='utf-8') as f:
            for line in f:
                if len(tmp_string_list) == int(self.num_words * mono_fraction):
                    break
                word = line.rstrip() #删除 string 字符串末尾的指定字符，（默认为空格）
...
def train(run_name, start_epoch, stop_epoch, img_w):
    ...
    model.fit_generator(
        ...
        callbacks=[keras.callbacks.EarlyStopping(patience=1),viz_cb, img_gen],
